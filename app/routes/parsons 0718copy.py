...
_PARSONS_HINT_PROMPT_VERSION = "structured_error_hint_v1_similarity_checked"
...
def _hint_relation_types_for_block(block, error_types):
    """Read pre-defined relation types without asking the LLM to infer them."""
    ...
    for error_type in (error_types or []):
        raw = str(error_type or "").strip().lower()
        if raw in {"sequence_error", "order_error", "order"}:
            normalized_errors.append("order")
        elif raw in {"indentation_error", "indentation"}:
            normalized_errors.append("indentation")
        elif raw:
            normalized_errors.append(raw)

    for error_type in normalized_errors:
        output.extend(_hint_string_list(relation_by_error.get(error_type)))

    # Generic relation fields are accepted as a fallback. These values must be
    # authored by the task-generation/curation pipeline, not generated here.
...
def _collect_all_wrong_slot_contexts(att, task):
    ...
    for slot_index in wrong_slots:
        ...
        relation_types = _hint_relation_types_for_block(expected_block, slot_errors)
...
def _build_aggregated_hint_detail(att, task, requested_hint_no=1):
    ...
    task_context = {
        ...
        "control_structure": str(task_doc.get("control_structure") or "").strip() or None,
    }
...
def _generate_ai_hint_payload(att, task, requested_hint_no):
    ...
    # The old concept/error-only library is disabled by default for the new
    # structured prompt. It can be explicitly re-enabled for compatibility,
    # but structured relation-aware retrieval should be implemented next.
    legacy_library_enabled = str(
    ...
    if legacy_library_enabled:
        ...
        hint_library_fields.update(
            _hint_library_status_fields(
                hint_library_ctx,
                "skipped",
                "legacy_concept_only_retrieval_disabled",
            )
        )
...
""".strip()
    ...
    # Structured prompts are intentionally not inserted into the old
    # concept/error-only library. Relation-aware retrieval and storage will be
    # added as a separate step to prevent cross-task hint contamination.
    if not hint_library_doc:
        structured_library_status = _hint_library_status_fields(
            hint_library_ctx,
            "skipped",
            "structured_relation_retrieval_not_implemented",
        )
        hint_library_fields.update(structured_library_status)
