...
from ..questionnaire import (
    ...
    has_questionnaire_response,
)
from ..randomization import is_test_data_user
...
_LEGACY_DEFAULT_TEST_CYCLE_ID = "default"
_DEFAULT_ASSESSMENT_VERSION = "v1"


def get_default_test_cycle_id() -> str:
    """預設測驗批次。v1.8 統一使用 test_control，不再依賴 parsons_test_cycles。"""
    return _DEFAULT_TEST_CYCLE_ID


def normalize_test_cycle_id(value) -> str:
    """將未提供或舊版 default 批次統一導向目前測驗批次。"""
    test_cycle_id = str(value or "").strip()
    if not test_cycle_id or test_cycle_id == _LEGACY_DEFAULT_TEST_CYCLE_ID:
        return get_default_test_cycle_id()
    return test_cycle_id


def get_student_test_cycle_id(student_id: str) -> str:
    """取得學生測驗批次；未個別指派時使用目前全班預設批次。"""
    sid = str(student_id or "").strip()
    if not sid:
        return ""
    user = db.users.find_one({"student_id": sid}, {"test_cycle_id": 1})
    if not user:
        return ""
    return str(user.get("test_cycle_id") or "").strip() or get_default_test_cycle_id()


def is_test_data_student(student_id: str) -> bool:
    """測試帳號只要已分配批次，即可驗證前測流程。"""
    sid = str(student_id or "").strip()
    if not sid:
        return False
    user = db.users.find_one(
        {"student_id": sid},
        {"student_id": 1, "is_test_data": 1},
    ) or {}
    return is_test_data_user(user, sid)


def can_access_pretest(student_id: str, test_cycle_id: str) -> bool:
    if not str(test_cycle_id or "").strip():
        return False
    return is_test_data_student(student_id) or is_pretest_open(test_cycle_id)


def _get_test_control_doc(test_role: str, test_cycle_id: str) -> dict:
    """讀取目前批次控制；新批次尚未建檔時相容既有 default 控制。"""
    role = "pre" if str(test_role or "").strip().lower() == "pre" else "post"
    field = "pre_open" if role == "pre" else "post_open"
    normalized_cycle_id = normalize_test_cycle_id(test_cycle_id)
    doc = db.test_control.find_one({"_id": f"{field}:{normalized_cycle_id}"}) or {}
    if doc or normalized_cycle_id != get_default_test_cycle_id():
        return doc
    return db.test_control.find_one({"_id": f"{field}:{_LEGACY_DEFAULT_TEST_CYCLE_ID}"}) or {}

def is_posttest_open(test_cycle_id: str) -> bool:
    """是否開放後測（統一讀 test_control）。"""
    doc = _get_test_control_doc("post", test_cycle_id)
...
def _canonical_block_meaning_zh(block: dict) -> str:
    """meaning_zh is canonical; legacy aliases are read only for compatibility."""
    if not isinstance(block, dict):
        return ""
    for key in ("meaning_zh", "semantic_zh", "semantic", "zh"):
    ...
    def _norm_blocks(blocks):
        ...
        for i, b in enumerate(blocks or []):
            if isinstance(b, dict):
                ...
                meaning = _canonical_block_meaning_zh(b)
                item = dict(b)
                item.update({
                    "id": str(bid),
...
#    - parsons_test_attempts: {student_id, test_cycle_id, test_role, task_id, is_correct, score, duration_seconds, submitted_at}
# =========================

# 前後測選擇題共用的計分規則。
_CHOICE_TEST_MAX_SCORE = 4
_CHOICE_TEST_MIN_SCORE = 0


def _pretest_questionnaire_required_response():
    return jsonify({
        "ok": False,
        "error": "pretest_questionnaire_required",
        "message": "請先完成前測問卷，再開始前測。",
        "next_path": "/pretest-survey",
...
def test_status():
    ...
    test_cycle_id = get_student_test_cycle_id(student_id)
    if not test_cycle_id:
        return jsonify({
            ...
            "post_done": False,
            "post_open": False,
            "pretest_questionnaire_completed": False,
            "pretest_questionnaire_required": False,
            "questionnaire_data_source": QUESTIONNAIRE_DATA_SOURCE,
            "message": "test_cycle_id is not assigned for this student",
        })

    pre_snapshot = _pretest_completion_snapshot(student_id, test_cycle_id)
    pre_done = bool(pre_snapshot.get("is_complete"))
    post_done = bool(db.parsons_test_attempts.find_one({"student_id": student_id, "test_cycle_id": test_cycle_id, "test_role": "post"}))
    pre_open = can_access_pretest(student_id, test_cycle_id)
    questionnaire_completed = has_questionnaire_response(db, student_id, test_cycle_id)
    questionnaire_required = bool(pre_open and not pre_done and not questionnaire_completed)
    ...
    return jsonify({
        ...
        "pre_open": pre_open,
        "pre_done": pre_done,
        "pretest_completed_tasks": pre_snapshot.get("completed_tasks", 0),
        "pretest_total_tasks": pre_snapshot.get("total_tasks", 0),
        "post_done": post_done,
        "post_open": is_posttest_open(test_cycle_id),
        "pretest_questionnaire_completed": questionnaire_completed,
        "pretest_questionnaire_required": questionnaire_required,
        "questionnaire_data_source": QUESTIONNAIRE_DATA_SOURCE,
    })

def is_pretest_open(test_cycle_id: str) -> bool:
    doc = _get_test_control_doc("pre", test_cycle_id)
...
def _choice_answer_key(question_doc: dict) -> str:
    ...
    if isinstance(question_doc, dict):
        raw = (
            question_doc.get("correct_answer")
            or question_doc.get("answer_key")
            or question_doc.get("answer")
...
def _choice_question_text(question_doc: dict) -> str:
    ...
    text = str(
        ...
        or question_doc.get("stem")
        or question_doc.get("instruction")
        or question_doc.get("question")
        or question_doc.get("title")
        or ""
    ).strip()
    # MongoDB Compass's document editor commonly saves a typed ``\\n`` as
    # two literal characters.  Accept both that notation and real line breaks
    # in author-entered choice-question text.
    return text.replace("\\r\\n", "\n").replace("\\n", "\n").replace("\\r", "\n")


def _choice_concept_tag(question_doc: dict) -> str:
    if not isinstance(question_doc, dict):
        return ""
    return str(
        question_doc.get("concept_tag")
        or question_doc.get("target_concept")
        or question_doc.get("concept")
        or question_doc.get("unit")
        or ""
    ).strip()


def _choice_test_data_source(test_role: str) -> str:
    role = str(test_role or "").strip().lower()
    if role == "pre":
...
def _active_test_question_docs(test_role: str) -> list:
    """Return the exact active source list used by the formal test endpoint."""
    role = str(test_role or "").strip().lower()
    collection_name = _test_question_collection_name(role)
    if not collection_name:
        return []

    if role == "pre":
        choice_docs = list(
            db.pre_parsons_questions.find(
                _active_test_question_query(_choice_test_question_types())
            ).sort(_test_question_sort_key())
        )
        if choice_docs:
            return choice_docs
    ...
    formal_docs = list(
        db[collection_name].find(_active_test_question_query()).sort(
            _test_question_sort_key()
        )
    )
    if formal_docs:
        return formal_docs

    # The current pretest supports a legacy fallback source of choice questions.
    if role == "pre":
        fallback_query = {
            "$and": [
                {"type": {"$in": _choice_test_question_types()}},
                {"active": {"$ne": False}},
                {"is_active": {"$ne": False}},
            ]
        }
        return list(db.questions.find(fallback_query).sort(_test_question_sort_key()))
    return []


def _expected_test_task_ids(test_role: str) -> list:
    ids = []
    for question_doc in _active_test_question_docs(test_role):
...
def _pretest_completion_snapshot(student_id: str, test_cycle_id: str) -> dict:
    expected_task_ids = _expected_test_task_ids("pre")
    expected_ids = set(expected_task_ids)
    attempts = list(db.parsons_test_attempts.find(
        {
            "student_id": student_id,
            "test_cycle_id": test_cycle_id,
            "test_role": "pre",
            "task_id": {"$in": expected_task_ids} if expected_task_ids else {"$exists": False},
        },
        {
            "task_id": 1,
            "score": 1,
    ...
    attempts_by_task = {
        ...
        for row in attempts
        if str(row.get("task_id") or "").strip() in expected_ids
    }
    completed_count = len(attempts_by_task)
    total_count = len(expected_task_ids)
    is_complete = bool(total_count and completed_count == total_count)

    score = 0.0
    max_score = 0.0
    for task_id in expected_task_ids:
        attempt = attempts_by_task.get(task_id) or {}
        score += _numeric_score(attempt.get("score"), 0.0)
        attempt_max = _numeric_score(attempt.get("max_score"), 0.0)
        if attempt_max <= 0:
            attempt_max = 4.0 if attempt.get("question_type") == "choice" else 1.0
        max_score += attempt_max
    percentage = round((score / max_score) * 100, 4) if max_score > 0 else 0.0
    return {
...
def _finalize_pretest_if_complete(student_id: str, test_cycle_id: str) -> dict:
    """Persist formal pretest completion without changing the import assignment."""
    snapshot = _pretest_completion_snapshot(student_id, test_cycle_id)
    result = {
        "pretest_completed": snapshot["is_complete"],
        "pretest_completed_tasks": snapshot["completed_tasks"],
        "pretest_total_tasks": snapshot["total_tasks"],
    }
    ...
    db.users.update_one(
        {"student_id": student_id, "role": "student"},
        {
            "$set": {
                ...
                "pretest_completed_at": completed_at,
                "pretest_completed_cycle_id": test_cycle_id,
                "pretest_score": snapshot["score"],
                "pretest_max_score": snapshot["max_score"],
                "pretest_score_percentage": snapshot["score_percentage"],
    ...
    user = db.users.find_one(
        {"student_id": student_id, "role": "student"},
        {"feedback_strategy": 1, "assignment_status": 1},
    ) or {}
    result.update({
        "feedback_strategy": user.get("feedback_strategy"),
        "assignment_status": user.get("assignment_status"),
        "next_path": "/home",
    })
    return result
...
def _parsons_question_to_task_doc(question_doc: dict) -> dict:
    ...
    for index, block in enumerate(raw_blocks):
        ...
        if not block_id:
            continue
        meaning = _canonical_block_meaning_zh(block)
...
def _build_test_question_response(question_doc: dict, total: int, current_index: int, test_role: str = ""):
    if _is_choice_test_question(question_doc):
        ...
        return {
            ...
            "concept_tag": _choice_concept_tag(question_doc),
            "options": _normalize_choice_options(question_doc),
            "total": int(total or 1),
            "current_index": int(current_index or 1),
            "data_source": _choice_test_data_source(test_role),
        }
...
def get_test_task():
    ...
    test_cycle_id = get_student_test_cycle_id(student_id)
    if not test_cycle_id:
        return jsonify({"ok": False, "message": "test_cycle_id is not assigned for this student"}), 403
    if test_role == "pre" and not can_access_pretest(student_id, test_cycle_id):
        return jsonify({"ok": False, "message": "pretest not open"}), 403
    if test_role == "pre" and not has_questionnaire_response(db, student_id, test_cycle_id):
        return _pretest_questionnaire_required_response()
    ...
    if question_doc:
        payload = _build_test_question_response(question_doc, total, current_index, test_role)
        payload["test_cycle_id"] = test_cycle_id
...
def _test_submission_json(payload: dict, student_id: str, test_cycle_id: str, test_role: str):
    """Attach pretest completion state to success and replay responses."""
    if test_role != "pre":
        return jsonify(payload)
    payload.update(_finalize_pretest_if_complete(student_id, test_cycle_id))
    return jsonify(payload)
...
def _submit_choice_test_answer(
    *,
    ...
    score = _CHOICE_TEST_MAX_SCORE if is_correct else _CHOICE_TEST_MIN_SCORE
    question_text = _choice_question_text(task)
    concept_tag = _choice_concept_tag(task)
    ...
    attempt_doc = {
        ...
        "test_cycle_id": test_cycle_id,
        "assessment_version": _DEFAULT_ASSESSMENT_VERSION,
        "test_role": test_role,
        "task_id": task_identity,
        "question_id": task_identity,
        "question_type": "choice",
        "question_text": question_text,
        "concept_tag": concept_tag or None,
        "attempt_no": 1,
        ...
        "score": score,
        "max_score": _CHOICE_TEST_MAX_SCORE,
        "min_score": _CHOICE_TEST_MIN_SCORE,
    ...
    try:
        ...
        }, student_id, test_cycle_id, test_role)
    except Exception as e:
    ...
    write_learning_log_safely({
        ...
        "attempt_no": 1,
        "target_concept": concept_tag or None,
...
def submit_test_answer():
    ...
    payload_duration_seconds = _safe_duration_seconds(data.get("duration_seconds"))
    if payload_duration_seconds is None:
        # Accept the former field temporarily so older browsers can still submit.
        payload_duration_seconds = _safe_duration_seconds(data.get("duration_sec"))
    payload_duration_sec = payload_duration_seconds
    ...
    if _is_choice_test_question(task):
        return _submit_choice_test_answer(
            ...
            test_cycle_id=test_cycle_id,
            test_role=test_role,
            payload_duration_seconds=payload_duration_seconds,
        )
...
def toggle_test_cycle():
    ...
    try:
        ...
        test_cycle_id = normalize_test_cycle_id(test_cycle_id)
...
def get_test_cycle_control():
    test_cycle_id = normalize_test_cycle_id(request.args.get("test_cycle_id"))
...
def export_test_csv():
    ...
    for d in cur:
        row = {
            ...
            "assessment_version": d.get("assessment_version", ""),
            "test_role": d.get("test_role", ""),
            "task_id": d.get("task_id") or d.get("test_task_id") or d.get("source_task_id") or "",
            "question_text": d.get("question_text") or d.get("task_title") or "",
            "concept_tag": d.get("concept_tag") or d.get("target_concept") or "",
            "selected_answer": d.get("selected_answer") or d.get("answer") or "",
            "selected_answer_text": d.get("selected_answer_text") or d.get("answer_text") or "",
            "correct_answer": d.get("correct_answer") or d.get("expected_answer") or "",
            "is_correct": d.get("is_correct", False),
            "score": d.get("score", ""),
            "max_score": d.get("max_score", ""),
            "min_score": d.get("min_score", ""),
            "duration_seconds": d.get("duration_seconds") if d.get("duration_seconds") is not None else d.get("duration_sec", ""),
            "wrong_indices": json.dumps(d.get("wrong_indices", []), ensure_ascii=False),
            "started_at": _taiwan_time_string(d.get("started_at") or d.get("started_at_utc")) or "",
            "submitted_at": _taiwan_time_string(d.get("submitted_at") or d.get("submitted_at_utc")) or "",
            "created_at": _taiwan_time_string(d.get("created_at") or d.get("created_at_utc")) or "",
            "updated_at": _taiwan_time_string(d.get("updated_at") or d.get("updated_at_utc")) or "",
        }
        w.writerow(row)
        ...
        if is_first_wrong_for_hint_flow:
            ...
            try:
                pending_record = _set_hint_record(
                    ...
                    task_id,
                    set_fields={
                        "group_type": _clean_string(user_profile_for_hint.get("group_type")), #根據學生群組類型，決定提示策略
                        "task_attempt_session": v2_doc.get("task_attempt_session"), #題目的作答
